        ...
        if (tableModel.getRowCount() < 1) {
            ...
            try {
                ...
                for (int i = 0; i < tableModel.getRowCount(); i++) {
                    String SelectKdKoleksi = "SELECT * FROM T_Peminjaman WHERE Kd_Peminjaman = '" + tableModel.getValueAt(i, 0) + "'";
                    ResultSet rs = stmt.executeQuery(SelectKdKoleksi);
                    if (rs.next()) {
                        getKd_Koleksi = rs.getString("Kd_Koleksi");
                        System.out.println(getKd_Koleksi);
                    }
                    String Ubah_TKoleksi = "UPDATE T_Koleksi SET Status ='TERSEDIA', Estimasi_Pengembalian = (NULL) WHERE Kd_Koleksi = '" + getKd_Koleksi + "'";
                    String Ubah_TPeminjaman = "UPDATE T_Peminjaman SET Tgl_Kembali ='" + Tanggal + "', Denda_Keterlambatan =" + tableModel.getValueAt(i, 3) + ", "
