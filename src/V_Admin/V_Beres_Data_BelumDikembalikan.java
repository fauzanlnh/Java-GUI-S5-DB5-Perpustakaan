...
public class V_Beres_Data_BelumDikembalikan extends javax.swing.JFrame {

    /**
     * Creates new form V_Beres_Data_BelumDikembalikan
     */
    Connection koneksi;
    C_Peminjaman Pinjam = new C_Peminjaman();
    C_Anggota Anggota = new C_Anggota();
    C_Koleksi Koleksi = new C_Koleksi();
    String getKdPeminjaman, getNamaAnggota, getKdKoleksi;
    int Keterlambatan;
    String getTglKembali;

    public V_Beres_Data_BelumDikembalikan() {
        initComponents();
    ...
    public void TampilData() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Keterlambatan = rs.getInt("Keterlambatan");
                ...
                dtm.addRow(new String[]{"" + No, getKdPeminjaman, getNamaAnggota, getKdKoleksi, "" + Keterlambatan});
                No = No + 1;
            }
        } catch (Exception ex) {
    ...
    public void CariData() {
        ...
        try {
            Statement stmt = koneksi.createStatement();
            Koleksi.setKdKoleksi(txtCari.getText());
            String Cari = Koleksi.getKdKoleksi();
            query = "SELECT Kd_Peminjaman, Nama_anggota, Kd_Koleksi, DATEDIFF(NOW(), T_Peminjaman.Estimasi_Pengembalian) AS 'Keterlambatan' "
                    + "FROM T_Peminjaman JOIN T_Koleksi USING(Kd_Koleksi) JOIN T_Anggota USING(Kd_Anggota) "
                    + "WHERE T_Peminjaman.Status ='DIPINJAM' "
                    + "AND Kd_Koleksi LIKE'%" + Cari + "%' ";
            ResultSet rs = stmt.executeQuery(query);
            int No = 1;
            while (rs.next()) {
                Pinjam.setKdPeminjaman(rs.getString("Kd_Peminjaman"));
    ...
    public void Move() throws ParseException {
        ...
        try {
            Statement stmt = koneksi.createStatement();
            Pinjam.setKdPeminjaman(tblPengembalian.getValueAt(tblPengembalian.getSelectedRow(), 1).toString());
            getKdPeminjaman = Pinjam.getKdPeminjaman();
            String select = "SELECT * FROM T_Peminjaman WHERE Kd_peminjaman = '" + getKdPeminjaman + "'";
            ResultSet rs = stmt.executeQuery(select);
            if (rs.next()) {
                Pinjam.setTglPinjam(rs.getString("Tgl_Pinjam"));
                Pinjam.setEsimasiKembali(rs.getString("Estimasi_Pengembalian"));
            }
            System.out.println(Tanggal);
            txtKodePeminjaman.setText(getKdPeminjaman);
            Koleksi.setKdKoleksi(tblPengembalian.getValueAt(tblPengembalian.getSelectedRow(), 3).toString());
            txtKodeKoleksi.setText(Koleksi.getKdKoleksi());
            //SET JDATE
            Date datePinjam = new SimpleDateFormat("yyyy-MM-dd").parse(Pinjam.getTglPinjam());
            Date dateEstimasi = new SimpleDateFormat("yyyy-MM-dd").parse(Pinjam.getEsimasiKembali());
            tglPinjam.setDate(datePinjam);
            tglKembali.setDate(dateEstimasi);
    ...
    public void TambahPengembalian() {
        ...
        String Tanggal = String.valueOf(fm.format(getTglKembali));
        Koleksi.setKdKoleksi(txtKodeKoleksi.getText());
        ...
        try {
            ...
            String Ubah_TPeminjaman = "UPDATE T_Peminjaman SET Tgl_Kembali ='" + Tanggal + "', Denda_Keterlambatan =0, Status = 'DIKEMBALIKAN', "
                    + "Estimasi_Pengembalian = (NULL) WHERE Kd_Peminjaman = '" + getKdPeminjaman + "'";
                ...
                .addGroup(mainPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(mainPanel4Layout.createSequentialGroup()
                        .addGroup(mainPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(tglPinjam, javax.swing.GroupLayout.PREFERRED_SIZE, 149, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void btnUbahMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnUbahMouseClicked
        if (tblPengembalian.getSelectedRow() >= 0) {
            try {
                ...
                Logger.getLogger(V_Beres_Data_BelumDikembalikan.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(V_Beres_Data_BelumDikembalikan.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new V_Beres_Data_BelumDikembalikan().setVisible(true);
            }
        });
    }
